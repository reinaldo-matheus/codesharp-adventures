import { javaPhases, getExercisesByPhase } from "@/data/javaLessons";
import { MapPin, Lock, CheckCircle2, Sparkles } from "lucide-react";

interface JavaWorldMapProps {
...
const nodePositions = [
  { x: 15, y: 82 }, // Phase 1 - Vila do Aprendiz
  { x: 38, y: 65 }, // Phase 2 - Academia da OO
  { x: 20, y: 42 }, // Phase 3 - Forja das Coleções
  { x: 45, y: 22 }, // Phase 4 - Torre das Exceções
  { x: 70, y: 32 }, // Phase 5 - Oficina de Testes
  { x: 60, y: 62 }, // Phase 6 - Cavernas da API
  { x: 85, y: 78 }, // Phase 7 - Pico do Deploy
];

export const JavaWorldMap = ({ currentPhaseId, completedExercises, totalExercises, onClose }: JavaWorldMapProps) => {
  ...
  const getExercisesProgressForPhase = (phaseId: number) => {
    const phaseExercises = getExercisesByPhase(phaseId);
    let exercisesBeforePhase = 0;
    for (let i = 1; i < phaseId; i++) {
      exercisesBeforePhase += getExercisesByPhase(i).length;
    }

    const completedInPhase = Math.max(0, Math.min(phaseExercises.length, completedExercises - exercisesBeforePhase));

    return { completed: completedInPhase, total: phaseExercises.length };
  };
        ...
        <div className="card-quest p-6 relative overflow-hidden">
          ...
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="font-cinzel text-2xl font-bold text-gold">Mapa do Backend</h2>
                <p className="text-sm text-muted-foreground">
                  {completedExercises}/{totalExercises} desafios completados
                </p>
              </div>
            </div>
            <button onClick={onClose} className="btn-rpg px-4 py-2 text-sm">
              Fechar
            </button>
          </div>
          ...
          >
            ...
            <div className="absolute inset-0 opacity-10">
              <svg className="w-full h-full">
                <defs>
                  <pattern id="java-grid" width="50" height="50" patternUnits="userSpaceOnUse">
                    <path d="M 50 0 L 0 0 0 50" fill="none" stroke="currentColor" strokeWidth="0.5" />
                  </pattern>
                </defs>
                <rect width="100%" height="100%" fill="url(#java-grid)" className="text-gold" />
              </svg>
            </div>
            ...
            {/* Region Nodes */}
            {javaPhases.map((phase, index) => {
              const pos = nodePositions[index];
              const status = getPhaseStatus(phase.id);
              const progress = getExercisesProgressForPhase(phase.id);
              ...
              return (
                ...
                >
                  ...
                  >
                    ...
                    <span
                      className={`
                      text-xs font-cinzel font-bold text-center whitespace-nowrap max-w-[90px] truncate
                      ${status === "completed" ? "text-success" : status === "current" ? "text-primary" : "text-muted-foreground"}
                    `}
                    >
