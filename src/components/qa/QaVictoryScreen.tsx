import { Trophy, Sparkles, Star, RotateCcw, ShieldCheck } from "lucide-react";

interface QaVictoryScreenProps {
  xp: number;
  level: number;
...
export const QaVictoryScreen = ({
  xp,
  level,
  correctAnswers,
  totalQuestions,
  onRestart,
}: QaVictoryScreenProps) => {
  ...
  return (
    <div className="card-quest text-center animate-bounce-in">
      ...
      {/* Title */}
      <h2 className="font-display text-3xl font-bold text-foreground mb-2">Certificação de Guardião(ã) QA!</h2>
      <p className="text-muted-foreground mb-8">
        Você completou toda a trilha de Qualidade, Automação de Testes e REST Assured!
      </p>
      ...
      <div className="bg-primary/10 border border-primary/30 rounded-lg p-4 mb-6">
        <p className="text-primary font-display font-semibold">
          {percentage >= 80
            ? "🎉 Excelente! Você já pensa e escreve testes como um QA de mercado!"
            : percentage >= 60
            ? "🛡️ Muito bom! Continue praticando REST Assured e automação!"
